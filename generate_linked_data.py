...
def generate_person_with_accounts_and_orders(fake, person_id, max_accounts, max_orders_per_account, org_uris, graph):
    ...
    graph.add((person_uri, SCHEMA.name, Literal(person_name, datatype=XSD.string)))
    graph.add((person_uri, SCHEMA.givenName, Literal(fake.person_given_name(), datatype=XSD.string)))
    graph.add((person_uri, SCHEMA.familyName, Literal(fake.person_family_name(), datatype=XSD.string)))
